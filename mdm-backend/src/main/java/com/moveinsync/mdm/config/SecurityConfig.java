...
@EnableConfigurationProperties(MdmSecurityProperties.class)
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/", "/dashboard.html", "/assets/**", "/favicon.ico", "/api-info").permitAll()
                .requestMatchers("/api/v1/device/**").permitAll()
                .requestMatchers("/api/v1/update/**").permitAll()
                .requestMatchers("/actuator/health").permitAll()
                .requestMatchers("/actuator/**").hasRole("ADMIN")
                .requestMatchers("/api/v1/admin/**").hasAnyRole("ADMIN", "RELEASE_MANAGER", "PRODUCT_HEAD")
